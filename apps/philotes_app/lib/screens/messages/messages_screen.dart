...
class MessagesScreen extends StatefulWidget {
  const MessagesScreen({
    super.key,
    this.messageService = const DevelopmentMessageService(),
  });
...
class _MessagesScreenState extends State<MessagesScreen> {
  ...
  List<MessageThread> get _threads {
    final values = widget.messageService
        .threads(_profile)
    ...
    // conversation order.
    //
    values.sort((a, b) => b.latestActivity.compareTo(a.latestActivity));

    return values;
  }

  bool _matchesSearch(MessageThread thread) {
    ...
    return thread.title.toLowerCase().contains(query) ||
        thread.latestPreview.toLowerCase().contains(query);
  }

  void _openThread(MessageThread thread) {
    widget.messageService.markThreadRead(thread.id);
  ...
  void _createMessage() {
    Navigator.of(context).push(
      MaterialPageRoute<void>(
        builder: (context) =>
            NewMessageScreen(messageService: widget.messageService),
      ),
    );
  }

  void _enterSelectionMode({MessageThread? selectedThread}) {
    setState(() {
      _selectionMode = true;

      if (selectedThread != null) {
        _selectedThreadIds.add(selectedThread.id);
      }
    });
  }
  ...
  void _toggleSelection(MessageThread thread) {
    setState(() {
  ...
  void _toggleSelectAll() {
    final visibleIds = _threads.map((thread) => thread.id).toSet();

    final allVisibleSelected =
        visibleIds.isNotEmpty && visibleIds.every(_selectedThreadIds.contains);

    setState(() {
      if (allVisibleSelected) {
        _selectedThreadIds.removeAll(visibleIds);
        ...
        _selectedThreadIds.addAll(visibleIds);
      }
    });
  }
  ...
  void _markSelectedRead() {
    ...
    for (final threadId in _selectedThreadIds) {
      widget.messageService.markThreadRead(threadId);
    }
  ...
  void _markSelectedUnread() {
    ...
    for (final threadId in _selectedThreadIds) {
      widget.messageService.markThreadUnread(threadId);
    }
  ...
  Widget build(BuildContext context) {
    ...
    final visibleIds = threads.map((thread) => thread.id).toSet();
            ...
            child: ConstrainedBox(
              ...
              child: Column(
                ...
                children: [
                  if (_selectionMode)
                    _SelectionHeader(
                      selectedCount: _selectedThreadIds.length,
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        PopupMenuButton<String>(
                          key: const Key('messagesOptionsMenu'),
                          icon: const Icon(
                            Icons.more_vert,
                            color: PhilotesColors.navy,
                          ),
                    ...
                    LayoutBuilder(
                      builder: (BuildContext context, BoxConstraints inner) {
                        final stacked = inner.maxWidth < 620;
                  ...
                  if (threads.isEmpty)
                    Container(
                      key: const Key('messageNoResults'),
                      ...
                      children: [
                        for (
                          ...
                          _ConversationCard(
                            ...
                            onOpen: () {
                              if (_selectionMode) {
                                _toggleSelection(threads[index]);
                              } else {
                                _openThread(threads[index]);
                              }
                            },
                            onLongPress: () {
                              if (!_selectionMode) {
...
class _SelectionHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      key: const Key('messageSelectionToolbar'),
        ...
        children: [
          Row(
            children: [
              ...
              TextButton(
                key: const Key('cancelMessageSelectionButton'),
                onPressed: onCancel,
          ...
          Row(
            children: [
              TextButton.icon(
                key: const Key('selectAllMessagesButton'),
                ...
                label: Text(allVisibleSelected ? 'Clear All' : 'Select All'),
              ),
          ...
          LayoutBuilder(
            builder: (BuildContext context, BoxConstraints constraints) {
              ...
              final markRead = OutlinedButton.icon(
                key: const Key('bulkMarkReadButton'),
                onPressed: selectedCount == 0 ? null : onMarkRead,
              ...
              final markUnread = FilledButton.icon(
                key: const Key('bulkMarkUnreadButton'),
                onPressed: selectedCount == 0 ? null : onMarkUnread,
              ...
              if (stacked) {
                return Column(
                  crossAxisAlignment: CrossAxisAlignment.stretch,
                  children: [markRead, const SizedBox(height: 10), markUnread],
                );
              }

              return Row(
                children: [
                  Expanded(child: markRead),
                  const SizedBox(width: 12),
                  Expanded(child: markUnread),
                ],
              );
            },
          ),
...
class _ConversationCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Material(
      ...
      child: InkWell(
        key: Key('messageThread-${thread.id}'),
        onTap: onOpen,
            ...
            children: [
              if (selectionMode) ...[
                Padding(
                  ...
                  child: Icon(
                    ...
                    key: Key('messageCheckbox-${thread.id}'),
                    color: selected
                        ? PhilotesColors.gold
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  color: PhilotesColors.navy,
                  border: Border.all(color: PhilotesColors.gold, width: 1.8),
                ),
                alignment: Alignment.center,
                child: Icon(
                  thread.isGroup ? Icons.groups_outlined : Icons.person_outline,
                  color: Colors.white,
                  size: 22,
                ),
              ),
