...
class NewMessageScreen extends StatefulWidget {
  const NewMessageScreen({super.key, required this.messageService});
...
class _NewMessageScreenState extends State<NewMessageScreen> {
  ...
  List<SuggestedMember> get _selectedFriends {
  ...
  bool get _isGroup => _selectedIds.length > 1;

  void _toggle(SuggestedMember friend) {
    setState(() {
  ...
  void _continue() {
    ...
    final selfName = _profile.displayName.trim().isEmpty
        ? 'You'
    ...
    final participants = <MessageParticipant>[
      MessageParticipant(
        ...
        initials: selfName.isEmpty
            ? 'Y'
            : selfName.substring(0, 1).toUpperCase(),
        isCurrentUser: true,
      ),
    ...
    final title = _isGroup
        ? _groupNameController.text.trim().isEmpty
              ? selected.map((friend) => friend.displayName).join(', ')
              : _groupNameController.text.trim()
        : selected.first.displayName;

    final thread = MessageThread(
      id: 'new-${DateTime.now().microsecondsSinceEpoch}',
      type: _isGroup ? MessageThreadType.group : MessageThreadType.direct,
      title: title,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      key: const Key('newMessageScreen'),
      ...
      appBar: AppBar(
        ...
        leading: IconButton(
          key: const Key('backFromNewMessageButton'),
          onPressed: () {
        ...
        title: Text(
          _isGroup ? 'New Group Message' : 'New Message',
              ...
              child: ConstrainedBox(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    TextField(
                      key: const Key('newMessageFriendSearch'),
                    ...
                    for (var index = 0; index < friends.length; index++) ...[
                      _FriendChoice(
                        ...
                        selected: _selectedIds.contains(friends[index].id),
                        onTap: () {
                          _toggle(friends[index]);
                        },
                      ),
                    ...
                    Text(
                      'Selected: '
                      '${_selectedIds.length}',
                      key: const Key('selectedFriendCount'),
                      style: const TextStyle(
                    ...
                    FilledButton(
                      key: const Key('continueNewMessageButton'),
...
class _FriendChoice extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Material(
      ...
      child: InkWell(
        key: Key('messageFriendChoice-${friend.id}'),
        onTap: onTap,
        ...
        child: Ink(
          ...
          decoration: selected
              ...
              : PhilotesDesign.secondaryCardDecoration(),
          child: Row(
            children: [
              Container(
                width: 44,
